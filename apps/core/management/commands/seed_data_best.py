...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not users:
            raise CommandError("User seeding failed: no users were created. Check your User model required fields.")
    ...
    def _fake_ar_job(self):
        # Some Faker Arabic locales do not provide the job() provider.
        provider = getattr(self.fake_ar, "job", None)
        if callable(provider):
            return provider()
        return random.choice([
            "مطور برمجيات", "محاسب", "مندوب مبيعات", "مصمم جرافيك",
            "مدير عمليات", "موظف خدمة عملاء", "مسؤول تسويق", "فني صيانة",
        ])
    ...
    def _seed_users(self, count, locs):
        ...
        for index in range(count):
            ...
            # Custom User models vary. Keep only fields that really exist.
            user, _ = User.objects.get_or_create(
                email=email,
                defaults=self._clean_kwargs(User, defaults),
            )

            for field, value in self._clean_kwargs(User, defaults).items():
                setattr(user, field, value)

            # If your User model still has username, keep it unique and deterministic.
            if "username" in self._model_field_names(User) and not getattr(user, "username", None):
                user.username = email.split("@")[0][:30]
        ...
        if not users:
            raise CommandError("No demo users were created.")
        ...
        agency, _ = RealEstateAgency.objects.get_or_create(
            ...
            defaults=self._clean_kwargs(RealEstateAgency, agency_defaults),
        )
        for field, value in self._clean_kwargs(RealEstateAgency, agency_defaults).items():
        ...
        RealEstateAgent.objects.get_or_create(
            user=users[0],
            defaults=self._clean_kwargs(RealEstateAgent, agent_defaults),
        )
    ...
    def _seed_jobs(self, users, locs, count):
        ...
        for index in range(count):
            ...
            self._set_trans(
                listing,
                {"title": self._fake_ar_job(), "description": self.fake_ar.paragraph(nb_sentences=3), "requirements": self.fake_ar.paragraph(nb_sentences=2)},
        ...
        self._set_trans(freelancer, {"title": self._fake_ar_job(), "bio": self.fake_ar.paragraph(nb_sentences=2)}, {"title": self.fake.job(), "bio": self.fake.paragraph(nb_sentences=2)})
